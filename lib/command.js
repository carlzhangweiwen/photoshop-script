...
//don't require ps here, as the circular dependencies will result in a {}
...
function stringify() { /* private dangler */
	...
	if (is(this.result_file, String)) {
		const result_file_code = os.platform() === 'darwin'
			? '\tvar file = File("' + path.join(os.tmpdir(), this.result_file) + '");\n'
...
module.exports = class Command {
	...
	constructor(func, options) {
		...
		this.options = Object.assign({}, ps.options, options);
	...
	execute() {
		...
		return ps.execute.apply(null, args);
	}

	executeSync() {
		const args = execute_args(this, arguments);
		return ps.executeSync.apply(null, args);
	}
};
